...
function smsg(BeraInstance, message, store) {
  if (!message) {
    return message;
  }
  ...
  if (message.key) {
    ...
    message.sender = BeraInstance.decodeJid(
      message.fromMe && BeraInstance.user.id || message.participant || message.key.participant || message.chat || ''
    );
    
    if (message.isGroup) {
      message.participant = BeraInstance.decodeJid(message.key.participant) || '';
    }
  }
  ...
  if (message.message) {
    ...
    if (quotedMessage) {
      ...
      message.quoted.sender = BeraInstance.decodeJid(message.msg.contextInfo.participant);
      message.quoted.fromMe = message.quoted.sender === BeraInstance.decodeJid(BeraInstance.user.id);
      ...
      message.quoted.getQuotedObj = message.quoted.getQuotedMessage = async () => {
        ...
        let quotedMsg = await store.loadMessage(message.chat, message.quoted.id, BeraInstance);
        return exports.smsg(BeraInstance, quotedMsg, store);
      };
      ...
      message.quoted["delete"] = () => BeraInstance.sendMessage(message.quoted.chat, { 'delete': quotedMessageFakeObj.key });
      message.quoted.copyNForward = (toChat, forceForward = false, options = {}) => BeraInstance.copyNForward(toChat, quotedMessageFakeObj, forceForward, options);
      message.quoted.download = () => BeraInstance.downloadMediaMessage(message.quoted);
    }
  }
  ...
  message.reply = (replyText, replyTo = message.chat, options = {}) => {
    if (Buffer.isBuffer(replyText)) {
      return BeraInstance.sendMedia(replyTo, replyText, "file", '', message, { ...options });
    }
    return BeraInstance.sendText(replyTo, replyText, message, { ...options });
  };
  ...
  message.copyNForward = (toChat = message.chat, forceForward = false, options = {}) => BeraInstance.copyNForward(toChat, message, forceForward, options);
